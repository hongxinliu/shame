...
Socket::~Socket() { stopAsyncReceiving(); }
...
void Socket::threadReceive() {
  ...
  socket_recv_.async_receive(
      ba::buffer(buffer.get(), max_len_packet_),
      boost::bind(&Socket::callbackReceive, this, buffer, ba::placeholders::bytes_transferred(),
