...
void Shame::callbackReceive(const std::string &channel,
                            ...
                            size_t size,
                            bool shared_memory) {
  // TODO(Hongxin): parallel subscriptions group & each subscription in group
  for (auto items : subscriptions_) {
    std::regex pattern(items.first);
    if (std::regex_match(channel, pattern)) {
      if (shared_memory) {
        const std::string key(reinterpret_cast<char*>(data.get()), size);
        ...
        for (auto &item : items.second) {
          item->callbackReceiveShm(channel, shame_data);
        }
      } else {
        for (auto &item : items.second) {
          item->callbackReceiveUdpm(channel, data, size);
        }
      }
    }
  }
